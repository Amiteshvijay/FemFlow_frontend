...
class _ChangeMobileScreenState extends State<ChangeMobileScreen> {
  ...
  Future<void> _sendOtp() async {
    ...
    try {
      await _profileService.sendMobileOtp(mobile);
      setState(() {
        _otpSent = true;
        _isLoading = false;
      });
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Mobile number changed successfully!"),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Mobile Number', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(24.0),
        child: _showForm ? _buildChangeForm() : _buildCurrentMobileView(),
      ),
    );
  }
}
